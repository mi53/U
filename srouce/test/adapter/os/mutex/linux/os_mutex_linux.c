...
//相对时间转换为相对于系统时间的绝对时间。
static ALM_VOID _Os_GetAbsTime(struct timespec * stAbsTime, ALM_UINT64 ulTimeOut);
...
ALM_INT32 OS_MutexCreate(CONST ALM_CHAR *pcName, OsMutexHandleSt* pstMutex)
{
    ...
    //初始化锁仓库的保护锁。
    iRet = pthread_mutex_init(&stMutexH.mutex, ALM_NULL);
    if (iRet != 0)
    {
    ...
    iRet = pthread_cond_init(&stMutexH.cond, ALM_NULL);
    if (iRet != 0)
    {
        pthread_mutex_destroy(&stMutexH.mutex);
        return RET_ERR;
...
{
    return OS_MutexPAftP(pstMutex, uiTimeOutInMillSec, ALM_NULL);
}
...
{
    OsMutexHSt* pstMutexH = (OsMutexHSt*)pstMutex->pOsExHandle;
    ALM_UINT64 ulThread = pthread_self();
    struct timespec stAbsTimeOut;
    ALM_INT32 iRet = RET_OK;

    if (ALM_NULL == pstMutexH)
    {
        return RET_ERR;
    }
...
static ALM_VOID _Os_GetAbsTime(struct timespec * stAbsTime, ALM_UINT64 ulTimeOut)
{
    ...
    gettimeofday(&stCurrentTime, ALM_NULL);
