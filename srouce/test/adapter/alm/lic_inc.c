...
/*将告警名称转为字符串*/
CONST ALM_CHAR* Tl_AlmAlarmName2Str(ALM_ENUM(ALM_ALARM_NAME_ENUM) eAlarmName)
{
	switch (eAlarmName)
	{
...
/* 将产品状态转换为字符串 */
CONST ALM_CHAR* Tl_AlmPrdState2Str(ALM_UINT32 uiState)
{
...
CONST ALM_CHAR* Tl_AlmState2Str(ALM_ENUM(ALM_STATE_LIC_ENUM) eState)
{
    ...
    case ALM_STATE_LIC_VALID:
        pcState = TL_ALM_STATE_LIC_VALID;
        break;
    ...
    ALM_UINT32 uiRsnSize)
{
    ...
    ALM_CallRet_M(ALM_NULL, (ALM_NULL));
...
{
    ALM_ChkRet_M(pcStr != ALM_NULL && peState != ALM_NULL, ALM_ERR_NULL_PTR, (ALM_NULL));

    if (0 == strcmp(pcStr, TL_ALM_STATE_INVALID))
    {
        *peState = ALM_STATE_LIC_INVALID;
        return ALM_OK;
    }

    if (0 == strcmp(pcStr, TL_ALM_STATE_GRACE))
    {
        *peState = ALM_STATE_LIC_GRACE_PERIOD;
        return ALM_OK;
    }

    if (0 == strcmp(pcStr, TL_ALM_STATE_LIC_VALID))
    {
        *peState = ALM_STATE_LIC_VALID;
        return ALM_OK;
    }

    ALM_LogErr_M(("Unknow state: %s", pcStr));
    ...
    ALM_ENUM(ALM_STATE_PRD_ENUM)* peState)
{
    ...
    if (0 == strcmp(pcStr, TL_ALM_STATE_DEFAULT))
    {
    ...
    if (0 == strcmp(pcStr, TL_ALM_STATE_DEMO))
    {
        *peState = ALM_STATE_PRD_DEMO;
        return ALM_OK;
    }

    if (0 == strcmp(pcStr, TL_ALM_STATE_NORMAL))
    {
        *peState = ALM_STATE_PRD_NORMAL;
        return ALM_OK;
    }
    
    if (0 == strcmp(pcStr, TL_ALM_STATE_EMERGENCY))
    {
        *peState = ALM_STATE_PRD_EMERGENCY;
        return ALM_OK;
    }

    if (0 == strcmp(pcStr, TL_ALM_STATE_STICK))
    {
